function Get-EmbeddedMsi {
  <#
    Extracts the embedded MSI payload from Affinity's installer exe by walking
    the PE resource directory (type -> ID -> language) and taking the largest
    binary resource leaf, rather than relying on hardcoded offsets that would
    break across version updates. Shared between chocolateyinstall.ps1 (runs on
    the end-user machine) and update.ps1 (runs at packaging time, so the MSI
    checksum can be computed and pinned alongside the exe's).
  #>
  param(
  ...
  try {
    ...
    $out = [System.IO.File]::Create($MsiFile)
